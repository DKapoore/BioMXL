...
/* ===== LS HELPER =====
   Now provided by js/data-store.js, which is backed by the real
   backend (GET/POST /api/data, /api/save) instead of localStorage,
   so admin changes persist across refreshes and for every visitor.
   The alias below keeps every existing LS.str(...) call working
   unchanged (data-store.js exposes it as LS.s). */
LS.str = LS.s;
...
function applyContactSettings() {
  ...
  // WhatsApp floating button customisation (show/hide, label, colour, icon)
  const waFloat = document.getElementById('waFloat');
  if (waFloat) {
    const waShow  = LS.str('biomxl_wa_show', '1');
    const waLabel = LS.str('biomxl_wa_label', 'WhatsApp');
    const waColor = LS.str('biomxl_wa_color', '');
    const waIcon  = LS.str('biomxl_wa_icon', '');

    waFloat.style.display = waShow === '0' ? 'none' : '';
    waFloat.title = waLabel;
    const labelEl = document.getElementById('waLabel');
    if (labelEl) labelEl.textContent = waLabel;
    if (waColor) waFloat.style.background = waColor;

    const defaultIcon = document.getElementById('waIconDefault');
    const customIcon  = document.getElementById('waIconCustom');
    if (waIcon) {
      if (defaultIcon) defaultIcon.style.display = 'none';
      if (customIcon) { customIcon.src = waIcon; customIcon.style.display = ''; }
    } else {
...
function renderFloatButtons() {
  document.querySelectorAll('.float-extra-btn').forEach(el => el.remove());
  const btns = LS.get('biomxl_float_btns', []);
  if (!Array.isArray(btns) || !btns.length) return;
  const waFloat = document.getElementById('waFloat');
  const waHidden = LS.str('biomxl_wa_show', '1') === '0';
  const baseBottom = 28;
  const step = 66; // spacing between stacked buttons
  btns.slice(0, 3).forEach((b, i) => {
    const a = document.createElement('a');
    a.className = 'float-extra-btn';
    a.href = b.link || '#';
    if (/^https?:\/\//i.test(b.link || '')) a.target = '_blank';
    a.style.background = b.color || '#1565c0';
    a.style.bottom = (baseBottom + (waHidden ? step * i : step * (i + 1))) + 'px';
    ...
    if (b.iconUrl) {
      ...
      span.className = 'fe-emoji';
      span.textContent = b.emoji || '🔗';
      a.appendChild(span);
    }
    if (waFloat && waFloat.parentNode) {
      waFloat.parentNode.insertBefore(a, waFloat);
    } else {
...
async function submitForm(e) {
  ...
  const message = document.getElementById('cf_msg').value.trim();
  ...
  const scriptUrl = LS.s('biomxl_leads_url', '');
  const recipients = LS.get('biomxl_leads_emails', []);

  if (scriptUrl) {
    try {
      await fetch(scriptUrl, {
        method: 'POST',
        mode: 'no-cors', // Google Apps Script Web Apps don't return CORS headers; the request still reaches the script.
        headers: { 'Content-Type': 'text/plain;charset=utf-8' },
        body: JSON.stringify({ name, mobile, email, state, message, source: 'Bio MXL Website', recipients })
      });
    } catch (err) {
      console.error('Lead submission failed:', err);
    }
  } else {
    console.warn('Lead capture Google Script URL not configured in Admin Panel → Lead Capture (Sheet).');
  }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // 0. Wait for real content to load from the backend (data/*.json)
  //    before rendering anything, so the page never briefly shows
  //    stale/default content.
  await window.LS_READY;
